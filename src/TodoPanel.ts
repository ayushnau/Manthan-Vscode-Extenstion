	...
	context.subscriptions.push(
		vscode.commands.registerCommand('catCoding.doRefactor', () => {
			if (TodoPanel.currentPanel) {
				TodoPanel.currentPanel.doRefactor();
			}
		})
	);
	...
	if (vscode.window.registerWebviewPanelSerializer) {
		...
		vscode.window.registerWebviewPanelSerializer(TodoPanel.viewType, {
			async deserializeWebviewPanel(webviewPanel: vscode.WebviewPanel, state: any) {
				...
				TodoPanel.revive(webviewPanel, context.extensionUri);
			}
		});
	}
}
...
export class TodoPanel {
	...
	private _getHtmlForWebview(webview: vscode.Webview, catGifPath: string) {
		...
		const scriptPathOnDisk = vscode.Uri.joinPath(this._extensionUri,'src','media', 'main.js');
        console.log({scriptPathOnDisk})
		// And the uri we use to load this script in the webview
		const scriptUri = webview.asWebviewUri(scriptPathOnDisk);

		// Local path to css styles
		const styleResetPath = vscode.Uri.joinPath(this._extensionUri,'src', 'media', 'reset.css');
		const stylesPathMainPath = vscode.Uri.joinPath(this._extensionUri,'src', 'media', 'vscode.css');
